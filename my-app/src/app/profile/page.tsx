...
const ProfilePage = () => {
  ...
  const account = searchParams.get('account'); // Fetch the account from query params
  const balance = searchParams.get('balance'); // Fetch the balance from query params
  ...
  const redeem = () => {
    ...
    const claimAmt = balance?.replaceAll('MM', '').trim();
    ...
    <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-br from-black to-gray-900 text-white relative overflow-hidden">
      ...
      <div className="bg-gray-900 bg-opacity-80 shadow-2xl rounded-lg p-10 max-w-2xl w-full transform hover:scale-105 transition-transform duration-500 ease-in-out">
        ...
        <div className="mb-6">
          ...
          <p className="mb-4 text-xl"><strong className="text-blue-400">Connected MetaMask Account:</strong> {account ? account : "Not connected"}</p>
          <p className="mb-6 text-xl"><strong className="text-yellow-400">Available Balance:</strong> {balance || '0 MM'}</p>
        </div>
        <button
          onClick={redeem}
      ...
      {showModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-80 z-50 transition-all duration-500 ease-in-out">
          <div className="bg-gray-800 text-white p-8 rounded-lg shadow-lg transform scale-95 animate-bounce">
            ...
            <p className="mt-4">Your {balance || '0 MM'} have been redeemed successfully.</p>
          </div>
        </div>
      )}
